...
namespace { std::unordered_map<std::string, std::vector<std::string>> g_docs; }
...
void PDFGenerator::addText(const std::string& docId, const std::string& text, int, int) { g_docs[docId].push_back(text); }
bool PDFGenerator::saveDocument(const std::string& docId, const std::string& filename) {
    std::ofstream out(filename);
    if (!out) return false;
    for (const auto& line : g_docs[docId]) out << line << '
';
    return true;
}
